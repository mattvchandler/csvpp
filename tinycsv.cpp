...
=='\r'||E)){while(!E){G if(c!='\r'&&c!=
'\n'){if(!E)ungetc(c,f);break;}}return{w
,1};}w+=c;}return{w,0};}I main(I c,char*
*v){if(c>2)return 1;FILE*f=0;if(c<2||v[1
]==S("-"))f=stdin;else f=fopen(v[1],"r")
;try{if(!f)throw 0;vector<vector<S>>d(1)
;vector<I>z;for(I i=0;;){auto[w,e]=p(f);
if(E)break;d.back().push_back(w);z.
